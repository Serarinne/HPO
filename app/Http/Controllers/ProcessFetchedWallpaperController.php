...
class ProcessFetchedWallpaperController extends Controller
{
    public function process(Request $request)
    {
        $fetchedId = $request->input('fetchedId');
        $targetId = $request->input('targetId');

        Log::info("[FetchedJob] Starting file processing for Fetched ID: {$fetchedId} Target ID: {$targetId}");
        ...
        try {
            $fetchedItem = FetchedWallpaper::find($fetchedId);
            $wallpaper = Wallpaper::findOrFail($targetId);
            ...
            Log::error("[FetchedJob] Failed processing fetch ID {$fetchedId}: " . $e->getMessage());

            $f = FetchedWallpaper::find($fetchedId);
            if ($f) {
